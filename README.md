...
This is a comprehensive SQL **Data Warehouse Project** consisting of an end-to-end **ETL Pipeline** implemented through the **Medallion Architecture** and use **Star Schema** for modelling.

This project is build upon fictional retail data generated by **LLMs** based on **Real Life Business Scenarios**
...
**Gold Layer :** Data is not extracted, but only access to data that has been combined and organized from the Silver layer is provided
...
2. **Enterprise Resource Planning (ERP)**
    ...
    - Additional Customer Demographics information `erp_cust_az12`
...
### Data Warehouse
The Medallion structure was selected for this particular project in order to establish a strict separation of concerns. 
...
- **Silver Layer**
  ...
  - Data is manipulated by **Cleaning** , **Standardizing** and **Normalizing** each column

  - Data **Enrichment** is also done through **Derived Columns**
 
  - Data **Relationship** between columns are established
...
- **Gold Layer**
  ...
  - Data transformation includes **Integration**, **Aggregations** and **Business Logics**

  - The **Fact** and **Dimension** tables are modelled to form a **Star Schema** structure to the data
...
### Documentation
Additional documentation includes **Data Catalog** with further breakdown of each tables and their columns of the final tables
...
Build an End-To-End Data Warehouse to convert raw data from retail store across different countries into actionable insights. 
The initial data is stored in the source as group of CSV files within two categories, Customer Relationship Management (CRM)
and Enterprise Resource Planning (ERP). Provide clear documentation for every step of the process to be shared with Analysts and
...
- [x] **Quality :** Clean, Standardize and Normalize the data for further processes

- [x] **Integration :** Combine and Connect the various elements in the data to form a analysis ready model
...
- **Are their any correlation between geographical area and product categories ?**
  - Geographical details are provided in the **Customers table and can be connected with the **Products**  
...
---
##  🔍 How To Run
The entire project was done inside **Microsoft SQL Server**, The **SQL** and **T-SQL** Syntax  
used for all the scripts are written in accordance with MS SQL Server compatibility.  
It is recommended to use MS SQL Server for running the scripts provided in this repository
...
2. The [`init_database`](scripts/init_database.sql) query will create a brand new database called **RetailDB** along with  
all the necessary schemas **bronze** , **silver** and **gold** within the database

3. The [`ddl_bronze`](scripts/bronze/ddl_bronze.sql) query followed by [`proc_bronze_load`](scripts/bronze/proc_bronze_load.sql) for creating bronze tables and loading data
